...
def craps(bet):
    rules = {
        '1': "Pass Line (1:1):\nВыигрыш, если в первом броске (Come-Out) выпадет 7 или 11.\nПроигрыш, если выпадет 2, 3 или 12.\nПри других значениях устанавливается 'точка' - нужно выбросить её повторно до выпадения 7.",
        '2': "Don't Pass (1:1, 12 - возврат):\nВыигрыш при 2 или 3 в первом броске.\nПроигрыш при 7 или 11.\nПри 12 - возврат ставки.\nПри установке 'точки' выигрыш, если 7 выпадет раньше точки.",
        '3': "Field (2:1 для 2, 3:1 для 12, 1:1 для 3,4,9,10,11):\nВыигрыш при 2,3,4,9,10,11,12.\nПроигрыш при 5,6,7,8.\nОсобые выплаты: 2 → 3:1, 12 → 2:1, остальные → 1:1.",
        '4': "Any Seven (4:1):\nВыигрыш при любом сочетании дающем 7.\nПроигрыш при всех других исходах.",
        '5': "Any Craps (7:1):\nВыигрыш при выпадении 'крэпса' (2, 3 или 12).\nПроигрыш при всех других исходах.",
        '6': "Hard Ways (8:1):\nВыигрыш при выпадении дубля (4,6,8,10) указанным способом до выпадения 'легкого' варианта или 7.\nПроигрыш, если число выпадет не дублем или выпадет 7.",
        '7': "Отменить ставку и вернуться в меню"
    }
    ...
    while True:
        ...
        print("7. Отменить ставку и вернуться")
        bet_type = input("> ")

        print(f"\n📜 Правила ставки:\n{rules[bet_type]}\n\n")

        if bet_type not in rules:
            print("⚠️ Неверный выбор. Пожалуйста, выберите 1-7.")
            continue
        if bet_type == '7':
            confirm = input("Вы точно хотите отменить ставку? (y/n): ").lower()
            if confirm == 'y':
                print("Ставка отменена. Возвращаемся в меню.")
                return 0
            else:
                continue
        else:
            confirm = input(f"Вы подтверждаете ставку '{bet_type}'? (y/n): ").lower()
            if confirm != 'y':
                print("Ставка отменена. Выберите другую ставку.")
                continue
            else:
                break
